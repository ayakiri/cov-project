...
def build_model(cnn: nn.Module,
                style_img: torch.Tensor,
                content_img: torch.Tensor) -> Tuple[nn.Sequential, List[StyleLoss], List[ContentLoss]]:
    """
    ...
    for layer in cnn.children():
        ...
        add_loss_layers(model, content_img, content_layers, style_img, style_layers, content_losses, style_losses, name)
...
def add_loss_layers(model: nn.Sequential,
...
def finalize_model(model: nn.Sequential,
                   content_losses: List[ContentLoss],
                   style_losses: List[StyleLoss]) -> Tuple[nn.Sequential, List[StyleLoss], List[ContentLoss]]:
    """
